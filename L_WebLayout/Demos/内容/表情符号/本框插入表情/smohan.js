$(function () {
    $.fn.smohanfacebox = function (options) {
        var defaults = {Event: "click", divid: "Smohan_FaceBox", textid: "TextArea"};
        var options = $.extend(defaults, options);
        var $btn = $(this);
        var faceimg = '';
        for (var i = 0; i < 60; i++) {
            faceimg += '<li><a href="javascript:void(0)"><img src="img/' + (i + 1) + '.gif" face="<emt>' + (i + 1) + '</emt>"/></a></li>';
        }
        ;$("#" + options.divid).prepend("<div id='SmohanFaceBox'>" +
            "<span class='Corner'></span>" +
            "<div class='Content'><h3><span>常用表情</span><a class='close' title='关闭'></a></h3><ul>" + faceimg + "</ul></div>" +
            "</div>");
    ...
    $.fn.extend({
        replaceface: function (faces) {
            for (i = 0; i < 60; i++) {
                faces = faces.replace('<emt>' + (i + 1) + '</emt>', '<img src="img/' + (i + 1) + '.gif">');
            }
            $(this).html(faces);
        }
    });
