...
(function ($) {
    $.fn.easyUpload = function (opts) {
        ...
        var FileOp = {
            ...
            formatFileSize: function (size, isJustKB) {
                if (size > 1024 * 1024 && !isJustKB) {
        ...
        this.each(function (index, element) {
            ...
            var easyManager = {
                init: function () {
                    var _html_txt;
                    _html_txt = '<div class="easy_upload-container"><div class="easy_upload-head"><input type="file" ';
                    _html_txt += option.multi ? 'multiple ' : '';
                    _html_txt += 'class="fileInput" data-count="0" style="display:none;" />';
                    _html_txt += '<span class="easy_upload_select noselect">' + option.selectText + '</span>';
                    _html_txt += option.multi ? '<span class="easy_upload_head_btn1 noselect">上传</span>' : '';
                    _html_txt += option.multi ? '<span class="easy_upload_head_btn2 noselect">删除</span>' : '';
                    _html_txt += option.multi ? '<i class="easyUploadIcon noselect head_check" data-checked="no">&#xe693;</i>' : '';
                    _html_txt += option.showNote ? '<span class="easy_upload_note">' + option.note + '</span>' : '';
                    _html_txt += '</div>';
                    _html_txt += '<ul class="easy_upload_queue"></ul>';
                    _html_txt += '</div>';
                    _ele.html(_html_txt);
                    this.bindHead();
                },
                bindHead: function () {
                    ...
                    $('.easy_upload_head_btn2').off('click').click(function () {
                        ...
                        if (arr.length > 0) _this._delFiles(arr, queueUl);
                    });
                },
                bindQueue: function () {
                    var _this = this;
                    ...
                    $('.easy_upload_delbtn').off('click').click(function () {
                        ...
                        if (upStatus !== '3') {
                            ...
                            _this._delFiles([indx], target);
                        }
                    });
                },
                _checkFile: function (fileArr, target) {
                    var typeArr = FileOp.getFileTypes(option.allowFileTypes);
                    if (typeArr.length > 0) {
                        for (var i = 0; i < fileArr.length; i++) {
                            var f = fileArr[i].file;
                            if (parseInt(FileOp.formatFileSize(f.size, true)) <= option.allowFileSize) {
                                fileArr[i].allow = $.inArray('*', typeArr) >= 0 || $.inArray(f.name.split('.').pop(), typeArr) >= 0;
                            } else {
                ...
                _renderFile: function (fileArr, target) {
                    ...
                    function render(file) {
                        ...
                        var _html_txt = '';
                        _html_txt += '<li class="easy_upload_queue_item" data-index="' + file.index + '">';
                        _html_txt += option.showPreview ? '<div class="easy_upload_preview queue_item-section">' + preview + '</div>' : '';
                        _html_txt += '<div class="easy_upload_file1 queue_item-section">';
                        _html_txt += '<p class="easy_upload_filename">' + f.name + '</p>';
                        _html_txt += '<p class="easy_upload_progress">';
                        _html_txt += '<span class="easy_upload_bar"></span>';
                        _html_txt += '</p>';
                        _html_txt += '</div>';
                        _html_txt += '<div class="easy_upload_file2 queue_item-section">';
                        _html_txt += '<p class="easy_upload_filesize">' + FileOp.formatFileSize(f.size) + '</p>';
                        _html_txt += '<p class="easy_upload_percent">0%</p>';
                        _html_txt += '</div>';
                        _html_txt += '<div class="easy_upload_status queue_item-section">';
                        _html_txt += file.allow ? sHtml : '<p class="status status6">文件不允许</p>';
                        _html_txt += '</div>';
                        _html_txt += '<div class="easy_upload_btn queue_item-section">';
                        _html_txt += file.allow ? '<p class="easy_upload_upbtn btn noselect">上传</p>' : '';
                        _html_txt += '<p class="easy_upload_delbtn btn noselect">删除</p>';
                        _html_txt += '</div>';
                        _html_txt += '<div class="easy_upload_checkone queue_item-section">';
                        _html_txt += option.multi ? '<i class="easyUploadIcon noselect queue_check queue_check_allow_' + file.allow + '" data-checked="no" data-up="1">&#xe693;</i>' : '';
                        _html_txt += '</div>';
                        if (option.multi) {
                            $(queueUl).append(_html_txt);
                        } else {
                            $(queueUl).html(_html_txt);
                        }
                    }
                ...
                _uploadFile: function (target) {
                    ...
                    function upload() {
                        if (allowNewPost) {
                            ...
                            if (option.formParam) {
                                for (let key in option.formParam) {
                ...
                _findItems: function (type, target) {
                    ...
                    if (type === 1) {
                        ...
                        icon = $(target).find('.queue_check[data-up="1"][data-checked="yes"]:visible,.queue_check[data-up="2"][data-checked="yes"]:visible,.queue_check[data-up="4"][data-checked="yes"]:visible');
                    }
                    for (var i = 0; i < icon.length; i++) {
                        var index = $(icon[i]).parent().parent().attr('data-index');
                        arr.push(index);
                    }
                    return arr;
                },
                _delFiles: function (arr, target) {
                    var _this = this;
                    ...
                    arr.forEach(function (item) {
                        ...
                        if (option.multi) del(item);
                        var qItem = _this._findEle(item, target);
                        if (qItem.upStatus === '2') delAllowFiles(item);
                    });
