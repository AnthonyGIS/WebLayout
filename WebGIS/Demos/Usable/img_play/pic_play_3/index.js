function Index() {
    ...
    this.decideDir = '';
    this.timer = null;
    this.settings = {slider_num: 4, w: 1200, h: 500, sliderDir: 'horizontal'};
}
...
Index.prototype = {
    setOpacity: function (index, fn) {
            ...
            if (this.settings.sliderDir === 'vertical' || this.settings.sliderDir === 'horizontal') {
                if (this.iNow === slider_len - 1) {
                    ...
                    this.slider_item.eq(0).css({
                        position: 'relative',[this.decideDir]: -this.setDirection() * slider_len
                    });
                } else {
                    this.iNow++;
                }
        ...
        for (var i = 0; i < this.settings.slider_num; i++) {
            imgStr += '<a href="#" imgTitle="Picture' + (i + 1) + '" class="slider-item"><img src="img/' + (i + 1) + '.jpg" alt="#"></a>';
